...
| 스킬 | 슬래시 명령 | 용도 |
|---|---|---|
| `setup-ib` | `/setup-ib` | 임의의 폴더/repo에 볼트 운영 컨텍스트(CLAUDE.md 블록 + `_system/`)를 셋업 — ib 스킬 첫 사용 전 1회 실행. 재실행 시 "싹 지우고 다시"(로컬) 옵션 |
| `init-vault` | `/init-vault` | 새 디렉토리에 볼트 전체 구조를 스캐폴딩 (보통 `/setup-ib`가 대신 호출) |
| `setup-gcp` | `/setup-gcp` | (선택) Google Sheets 미러용 GCP 자격증명을 계정당 1회 프로비저닝 — 프로젝트(기본 `infinite-brain`)·서비스계정·키, 멱등(있으면 재사용) |
| `setup-sheets-sync` | `/setup-sheets-sync` | (선택) 현재 볼트를 Google Sheet(기본 `지식`)에 연동 — 시트 생성·공유·템플릿 복사·gh 연동·초기 동기화 |
...
- **Google Sheets 미러** — 볼트를 구글 시트로 단방향 동기화(노드 `_data` + 관계 `_edges` 탭, push 시 변경분만). 필터·대시보드·공유, 대화형 에이전트 그라운딩용. `/setup-gcp`(자격증명 1회) → `/setup-sheets-sync`(볼트별 연동) 순으로 설정. 템플릿·근거: [`setup-sheets-sync/`](./setup-sheets-sync/).
