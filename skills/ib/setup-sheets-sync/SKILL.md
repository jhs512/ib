...
## 1. Choose the target spreadsheet (default: a new `지식` sheet)
...
Prefer the **Google Drive MCP tools** (no browser needed). If they aren't available, fall back to driving the browser to `sheets.new`.
...
## 2. Share the spreadsheet with the service account — ★ human only
...
## 3. Copy the sync templates into the vault repo
