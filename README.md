<p align="center">
  <img src="https://img.shields.io/badge/Version-0.5.2-brightgreen.svg" alt="Version">
...
```shell
/plugin marketplace update ib      # 마켓플레이스 메타데이터를 GitHub에서 새로 당겨옴
/plugin update infinite-brain@ib   # 설치된 플러그인을 최신 버전으로 갱신
```

- 업데이트 후 새 스킬이나 변경이 현재 세션에 바로 안 보이면 `/reload-plugins`(또는 세션 재시작)로 로드합니다.
- `/plugin marketplace update`가 "no changes"로 나오면 마켓플레이스 캐시가 오래된 것 — `/plugin marketplace remove ib` 후 `/plugin marketplace add jhs512/ib`로 다시 추가하면 최신 버전을 확실히 가져옵니다.
