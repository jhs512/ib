<p align="center">
  <img src="https://img.shields.io/badge/Version-0.5.3-brightgreen.svg" alt="Version">
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg" alt="License"></a>
  <img src="https://img.shields.io/badge/Claude_Code-Plugin-purple.svg" alt="Claude Code Plugin">
  <img src="https://img.shields.io/badge/Skills-8-orange.svg" alt="8 Skills">
...
| 명령 | 하는 일 |
|---|---|
| `/setup-ib` | 임의의 repo/폴더에 볼트 운영 컨텍스트 셋업 (최초 1회). 기본으로 git repo도 셋업(물어본 뒤 `git init` + 선택적 GitHub repo). 재실행 시 "싹 지우고 다시"(로컬) 옵션 |
...
> 세션 안에서 `/plugin` 은 **대화형 플러그인 매니저 UI를 엽니다**(Discover/Installed/Marketplaces/Errors 탭). `/plugin update ...` 처럼 인자를 붙여도 인자는 무시되고 UI만 열립니다 — 업데이트는 **UI 안에서** 하거나 **터미널 CLI**(`claude plugin …`)로 합니다. **Claude Code를 끌 필요는 없습니다.**

**방법 A — 대화형 UI (`/plugin`):**
1. **Marketplaces** 탭 → `ib` 새로고침(GitHub에서 최신 메타데이터를 당겨옴).
2. **Installed** 탭 → `infinite-brain` 선택 → 업데이트.

**방법 B — 터미널 CLI:**
```shell
claude plugin update infinite-brain@ib   # <plugin>@<marketplace>
```
"already at the latest version" 이 뜨면 마켓플레이스 캐시가 옛 버전인 것 — UI **Marketplaces** 탭에서 `ib` 를 새로고침하거나, 제거 후 `claude plugin marketplace add jhs512/ib` 로 재추가하세요.

**현재 세션에 반영:** 업데이트는 새 버전을 디스크에 설치할 뿐, 돌고 있는 세션은 이미 로드된 옛 버전을 씁니다. 새 버전을 적용하려면 **`/reload-plugins`** (안 되면 세션 재시작).
